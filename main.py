...
async def reminder(ctx, task, timings):
    ...
    branch_embed.set_thumbnail(url="{0}".format(
        image_link[randrange(0, image_length)]))
    branch_embed.add_field(name="**Information:**",
                           value="**Reminder set to: **" + str(task), inline=False)
    branch_embed.set_footer(text="{0}".format(
        footer_text[randrange(0, footer_length)]))
    while True:
        now = datetime.now().strftime("%H%M%S")
        hour = int(now[0:2])
        min = int(now[2:4])
...
async def reminder_info(ctx):
    ...
    info_embed.add_field(
        name="Format:", value="`------------------`", inline=False)
    info_embed.add_field(name='!bot reminder "your task" time in s/m/h ',
                         value="`------------------`", inline=False)
