...
class Moderation(commands.Cog):
	...
	async def ban(self,ctx,members:discord.Member,reason=None):
		...
		ban_embed=await displayEmbed(ctx, title="**BANNED**",desc="User{members} has been banned by {ctx.author.name}",colour=discord.Colour.dark_gold())
	...
	async def kick(self,ctx, members:discord.Member, reason=None):
			...
			channel= self.client.get_channel(648197664659341394)
			kick_embed=await displayEmbed(ctx, title="**KICKED**",desc=f"User{members} has been kicked by {ctx.author.name}",colour=discord.Colour.dark_gold())
